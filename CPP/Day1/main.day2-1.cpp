...
int main()
{
    ...
    {   
        ...
        int gameId = stoi(gameStr.substr(line.find(' '), line.length()));
        ...
        while (true) 
        {
            ...
            while (true) 
            {
                ...
                cout << "NUMBER OF CUBES: "<<numCubes << endl;
                string colourStr = play.substr(play.find_last_of(' ')+1, play.length());
                
                int index = 0;
                for(const string &checkColour: targetColours)
                {
                    cout << "checking color : " << checkColour << " : " << colourStr << endl;
                    if(checkColour == colourStr)
                    {
                        if(numCubes > targetCubeRgb[index])
                        {
                            cout << "********** GAME NOT POSSIBLE************" << endl;
                            gamePossible = false;
                        }
                    }
                    index++;
            ...
            int index = 0;
            for(const int &numberOfCubes: currentRgb)
            {
               if(numberOfCubes > targetCubeRgb[index])
                    gamePossible = false;
                
                index++;
            }
            
            if(!gamePossible || pos == std::string::npos )
                break;
